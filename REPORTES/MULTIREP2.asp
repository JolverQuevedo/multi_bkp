...
FECEMB = Request.QueryString("FECEMB") 
...
FOR I= 1 TO UBOUND(aVal)
   if len(trim(aVal(i))) > 0 then
       if trim(UCASE(aDat(i))) = "FECDOC" then
           if ini = fin then cad = cad else cad = cad + " and fecdoc between Convert(datetime,'"&ini&"',103)  and Convert(datetime,'"&fin&"',103) "
        else
            if trim(UCASE(aDat(i))) = "FECEMB" then
                if FECEMB = fin and FECEMB = ini then cad = cad else cad = cad + " and FECEMB  = Convert(datetime,'"&FECEMB&"',103) "
