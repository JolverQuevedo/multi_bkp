...
{	// busca un dato en un array
    ...
    j = 0
    for (i = 1; i < arr.length; i++) {   //alert(trim(arr[i]))
...
{	// busca el elemento activo en un pop down dependiendo del valor
	var a = '';
...
function ubicasele(subcadena, elemento) {	// busca el elemento activo en un pop down dependiendo del descriptor
    var a = subcadena;
    
    for (rt = 0; rt < elemento.length; rt++) {
        if (elemento.options(rt).text == a)
        { return (rt); }
        if (trim(elemento.options(rt).text) == trim(a))
        { return (rt); }
...
{	// devuelve una cadena con elementos válidos para SQL
  ...
  {	ch = checkString.substring(i, i+1);
    ...
    cad = cad + '(ch == "Ñ")||' ;
    cad = cad + '(ch == "ª")||' ;
    cad = cad + '(ch == "Ú")||(ch == ".") || (ch == "Ú")||(ch == "@")||(ch == "$")||(ch == "(")||(ch == ")")||(ch == "=")';
    if (eval(cad)== true )
    {  newString += ch;      }
    else
    {  newString += " ";}
