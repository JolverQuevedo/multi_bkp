...
//    alert(mul)
if (mul != 0) {
    cad = '../fichaposMOVI.asp?cod=' + '<%=cod%>' + '&cli=' + '<%=trim(request.querystring("cli"))%>'
//   alert(cad)
    window.parent.window.location.replace(cad)
    kam = '../detaacabados.asp?pos=' + '<%=trim(request.querystring("cli"))%>'
    kam += '&id=' + '<%=trim(cod)%>'
    var si
    si = confirm(kam);
    if (si == true)
