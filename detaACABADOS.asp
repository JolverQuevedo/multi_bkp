...
<!--#include file="COMUN/COMUNDETAACABADOS.ASP"-->
...
CAD =	" SELECT TOP "&PAGESIZE&" [ID] , PO,             " & _
        " isnull(fecemb,'') as FECEMB, CORTE, UBI,       " & _
        " ESTCLI,  TEMPO, COLOR,MOV, GUIA, FECGUI1, GUIA2, " & _
        " FECGUI2, NOMPRO AS TALLER,  DESSER,  bolsa as O_Srv,   " & _
        " (C0+C1+C2+C3+C4+C5+C6+C7+C8+C9) AS TOT         " & _
        " FROM VIEW_POS_movi AS T1                       " & _
		" WHERE CODCLI  = '"&POS&"'  and                 " & _
        " not corte = '' and year(fecdoc) > 2015 and "
...
function guia() {
    window.open('FichaGuiaMOVI.asp?cli=' + '<%=pos%>', "Guia_Acabados", opc)
    return true;
}
function resumen() {
